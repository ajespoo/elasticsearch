...
import org.elasticsearch.http.netty.HttpMessageEvent;
import org.elasticsearch.http.netty.HttpUtils;
...
public class CorsHandler extends SimpleChannelUpstreamHandler {
    ...
    public void messageReceived(final ChannelHandlerContext ctx, final MessageEvent e) throws Exception {
        ...
        if (config.isCorsSupportEnabled() && e.getMessage() instanceof HttpRequest) {
            ...
            if (RestUtils.isBrowser(request.headers().get(USER_AGENT))) {
                ...
                // If we have to set some CORS response headers, create the response now and pass it
                // along the pipeline through the message event, to be used by downstream handlers.
                HttpResponse resp = null;
                String originHeader = request.headers().get(ORIGIN);
                if (!Strings.isNullOrEmpty(originHeader)) {
                    if (config.isOriginAllowed(originHeader)) {
                        resp = HttpUtils.newResponse(request);
                        resp.headers().add(ACCESS_CONTROL_ALLOW_ORIGIN, originHeader);
                    }
                }
                ...
                if (resp != null) {
                    messageEvent = HttpMessageEvent.newEvent(e, resp);
                }
            }
        }
    ...
    private void setAllowMethods(final HttpResponse response) {
        response.headers().set(ACCESS_CONTROL_ALLOW_METHODS,
            config.allowedRequestMethods().stream().map(HttpMethod::getName));
    }
